...
import { isSortableItemPayload, DraxSnapbackTargetPreset } from './types';
...
export const DraxList = <T,>(props: DraxListProps<T>) => {
  ...
  const cellSpringConfig = useMemo(
    () => resolvedAnimConfig.useSpring
      ? { damping: resolvedAnimConfig.springDamping, stiffness: resolvedAnimConfig.springStiffness, mass: resolvedAnimConfig.springMass }
      : null,
    [resolvedAnimConfig],
  );

  const { hoverReadySV, hoverClearDeferredRef, dragPhaseSV, draggedIdSV, hoverPositionSV, hoverDimsSV, setHoverContent, isDragAllowedSV } = useDraxContext();
  ...
  // ── Item measurement ──
  const handleItemLayout = useCallback((itemKey: string, height: number) => {
    ...
    if (changed) {
      ...
      if (int.isDraggingRef.current) {
        ...
        int.itemHeightsSV.value = { ...int.itemHeightsSV.value, [itemKey]: height };
  ...
  // ── Cell recycler ──
  const updateVisibleCells = useCallback((scrollOffset: number) => {
    ...
    for (const key of keys) {
      ...
      if (!basePos) { missingBaseCount++; visibleKeys.add(key); continue; }
  ...
  }, [int, horizontal, screenWidth, screenHeight, drawDistance, estimatedItemSize]);
  ...
  useEffect(() => {
    ...
    if (hoverClearDeferredRef.current) {
      ...
      })(hoverReadySV, dragPhaseSV, draggedIdSV, hoverPositionSV, hoverDimsSV, isDragAllowedSV, setHoverContent);
    }
  }, [data]);
  ...
  const dropIndicatorInfoRef = int.dropIndicatorInfoRef as React.RefObject<DropIndicatorInfo<T> | undefined>;
  ...
  const onMonitorDragStart = useCallback((eventData: DraxMonitorEventData) => {
    ...
    if (renderDropIndicator) {
      ...
      lastIndicatorSlotRef.current = displayIdx >= 0 ? displayIdx : originalIndex;
  ...
  }, [int, keyExtractor, renderDropIndicator, dropIndicatorPositionSV, dropIndicatorVisibleSV, estimatedItemSize]);

  // ── Auto-scroll interval ──
  const autoScrollRef = useRef<{ interval: ReturnType<typeof setInterval>; direction: 'back' | 'forward' } | null>(null);
  ...
  const startAutoScroll = useCallback((direction: 'back' | 'forward') => {
    ...
    const doScroll = () => {
      ...
      const containerSize = horizontal ? containerMeas.width : containerMeas.height;
      const jump = containerSize * 0.2;
      const current = int.scrollOffsetSV.value;
      const target = direction === 'back'
        ? Math.max(0, current - jump)
        : current + jump;
  ...
  const onMonitorDragOver = useCallback((eventData: DraxMonitorEventData) => {
    ...
    const isOverThisList =
      absPos.x >= containerMeas.x && absPos.x <= containerMeas.x + containerMeas.width &&
      absPos.y >= containerMeas.y && absPos.y <= containerMeas.y + containerMeas.height;
    ...
    if (workletHandlesShifts) {
      ...
      const contentX = absPos.x - containerMeas.x + (horizontal ? scrollOffset : 0);
      const contentY = absPos.y - containerMeas.y + (horizontal ? 0 : scrollOffset);
    ...
    if (workletHandlesShifts) {
      ...
      if (!dropIndicatorVisibleSV.value) {
        ...
        if (renderDropIndicator) {
          ...
          const item = dataIdx !== undefined ? int.dataRef.current[dataIdx] : undefined;
          if (item) {
  ...
  }, [int, horizontal, startAutoScroll, stopAutoScroll, renderDropIndicator, dropIndicatorPositionSV, dropIndicatorVisibleSV, estimatedItemSize]);
  ...
  const handleDragEnd = useCallback((): DraxProtocolDragEndResponse => {
    ...
    if (basePos && containerMeas) {
      ...
      return {
        x: containerMeas.x + visualX - (horizontal ? int.scrollOffsetSV.value : 0),
        y: containerMeas.y + visualY - (horizontal ? 0 : int.scrollOffsetSV.value),
      };
    }
  ...
  const onMonitorDragEnd = useCallback((_eventData: DraxMonitorEndEventData): DraxProtocolDragEndResponse => {
    return handleDragEnd();
  }, [handleDragEnd]);

  const onMonitorDragDrop = useCallback((_eventData: DraxMonitorDragDropEventData): DraxProtocolDragEndResponse => {
    return handleDragEnd();
  }, [handleDragEnd]);

  // ── onSnapEnd: the proper commit point (called by DraxView after snap animation) ──
  const handleSnapEnd = useCallback((snapData: DraxSnapEndEventData) => {
  ...
  const totalSize = int.totalContentSizeRef.current ||
    (numColumns > 1
  ...
  const cellWidthForGrid = containerWidth > 0
    ? (numColumns > 1 ? containerWidth / numColumns : containerWidth)
    : undefined;
  ...
  const itemAlignSelf = useMemo(() => {
    const flat = contentContainerStyle ? StyleSheet.flatten(contentContainerStyle) : undefined;
    return (flat?.alignItems ?? 'stretch') as 'flex-start' | 'center' | 'flex-end' | 'stretch';
  }, [contentContainerStyle]);
  ...
  return (
    ...
    >
          ...
          >
            {bindings.map((binding) => {
              ...
              const itemCellHeight = horizontal
                ...
                : (getItemSpan ? dims?.height : undefined);
      ...
      {renderDropIndicator && <DropIndicatorOverlay
...
const DropIndicatorOverlay = <T,>({
  ...
  const style = useAnimatedStyle(() => {
    ...
    const springConfig = !isNewDrag && animConfig.useSpring
      ? { damping: animConfig.springDamping, stiffness: animConfig.springStiffness, mass: animConfig.springMass }
      : undefined;
  ...
  const overlayLayout = info ? { width: info.width, alignItems: itemAlignSelf as 'flex-start' | 'center' | 'flex-end' | 'stretch' } : undefined;
