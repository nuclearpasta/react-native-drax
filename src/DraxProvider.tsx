...
export const DraxProvider = ({
	...
	const { getViewState, getTrackingStatus, getAllViewIds, dispatch } =
		useDraxState();
	const {
		getAbsoluteViewData,
		getTrackingDragged,
		...
		(dragPositionData: TDragPositionData) => {
			...
			if (dragged && dragPositionData) {
				...
				if (monitors.length > 0 || prevMonitorIds.length > 0) {
					...
					prevMonitorIds
						.filter(
							(monitorId) => !newMonitorIds.includes(monitorId),
						)
						.forEach((monitorId) => {
			...
			(event: GestureStateChangeEvent<PanGestureHandlerEventPayload>) => {
				...
				if (dragged) {
					...
					if (receiver && shouldDrop) {
						...
						let response =
							draggedData.protocol.onDragDrop?.(eventData);
						...
						response =
							receiver.data.protocol.onReceiveDragDrop?.(
								eventData,
							);
						...
						let responded = false;
						let response =
							draggedData.protocol.onDragEnd?.(eventData);
						if (response !== undefined) {
							snapbackTarget = response;
							responded = true;
						}
