...
Grid span for a sortable item — how many columns and rows it occupies. Used with `getItemSpan` on `useSortableList` to enable mixed-size grid layouts.

```tsx
interface GridItemSpan {
